...
public class MainService {
    ...
    public void addVacancy(String title, String description, String requirements, Customer customer) {
        Vacancy vacancy = new Vacancy(customer, parseRequirement(requirements),
    ...
    public void updateVacancy(Vacancy toReplace, String title, String description, String requirements) {
        if (!Stream.of(title, description, requirements).allMatch(Strings::isBlank)) {
            ...
            if (!isBlank(requirements)) {
                toReplace.setRequirements(parseRequirement(requirements));
            }
            repo.addVacancy(toReplace);
        }
    }

    private List<Requirement> parseRequirement(String requirements) {
        return Stream.of(requirements.split("\n"))
                .map(req -> new Requirement(req.trim()))
                .collect(Collectors.toList());
    }
}
